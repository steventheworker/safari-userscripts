...
function removeButtons() {
  ...
  const btn = doc.querySelector(".ytp-right-controls button").nextSibling;
...
function sortByNew(sortByOld) {
  ...
  doc.querySelectorAll("#sort-menu a")[sortByOld || isWatching ? 1 : 2].click();
}
function sortByTop() {
  commentSortType = "top";
...
function ListenEvents() {
  ...
  win.addEventListener(
    ...
    function (e) {
      ...
      if (e.key === "l" || e.key === "j" || e.key === "d") e.stopPropagation(); //prevent default / map to Arrow's (5 sec fast-forward/rewind, rather than 10 sec)
  ...
  win.addEventListener("keydown", function (e) {
    ...
    function clickItem(menuIndex, itemIndex) {
      ...
      doc
        .querySelectorAll("#filter-menu ytd-search-filter-group-renderer")
  ...
  win.addEventListener("dblclick", function (e) {
    ...
    if ($relatedSection) {
      ...
      $relatedSection.classList[
        $relatedSection.classList.contains("cancelSpoiler") ? "remove" : "add"
      ]("cancelSpoiler");
    }
  });
  doc.querySelector("input.yt-searchbox-input").addEventListener(
    "keydown",
    (e) => {
...
function addStyleSheet() {
  const css = `
    ...
    ytd-live-chat-frame  iframe, ytd-live-chat-frame {width: 500px !important;}
	 ...
	 ytd-watch-flexy[theater-requested_]:not([fullscreen]) #full-bleed-container {
...
function mobileInit() {
  ...
  function trytrytry(tries = 0) {
    ...
    if (tries === 3) return;
    setTimeout(() => {
      if (!checkOpenYoutubeBanner()) trytrytry((tries.target ? 0 : tries) + 1);
    }, 500);
  }
