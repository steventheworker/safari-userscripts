...
export function init() {
	setTimeout(() => {
		...
		refocus();
		(focusedCached as HTMLInputElement).focus(); //reset focus on input, if it was there before
	}, 500);
}

const getToolPopupContainer = () => {
	// const testList = Array.from(doc.body.children).slice(-45); //firefox ~21st, safari ~42nd
	// const testPassers = testList.filter((el) => {
	// 	return el.querySelectorAll("a").length;
	// });
	// return testPassers[testPassers.length - 1];
	return doc.querySelector("#result-stats")!.parentElement!.parentElement!
...
export function keydown(e: KeyboardEvent) {
	...
	const el_index_dict: { [key: string]: number } = {
		Y: 5,
		M: 4,
		W: 3,
		D: 2,
	...
	if (el_index_dict[e.key] !== undefined) {
		...
		//click Any time ▾ btn
		(
			getToolPopupContainer().querySelectorAll(
				'div[role="button"]',
			)[0] as HTMLButtonElement
		).click();
		// click "Past Year ▸" button in popup1 (Tools ▾)
		setTimeout(() => {
			let lastToolPopup = getToolPopupContainer();
			lastToolPopup = lastToolPopup.children[
				lastToolPopup.children.length - 1
			] as HTMLElement;
			(
				lastToolPopup.querySelectorAll("a")[
					el_index_dict[e.key]
...
let query: HTMLElement | NodeListOf<HTMLElement> | HTMLCollection | null,
	ray: Array<HTMLElement>; //filtered query ^^^
let i = 0; // focus on results (a & d keys)
const $searchForm = () =>
	doc.getElementById("searchform") || doc.getElementsByTagName("header")[0]; //#searchform DNE on mobile
//used to determine if isMobile
const searchBtn: () => HTMLButtonElement = () => {
	const el: HTMLButtonElement | null = doc.querySelector(
		"button[type='submit']",
	);
	if (!el || el.disabled) {
...
function findParentResult(tar: HTMLElement) {
	if (!tar || !tar.parentElement) return null;
	if (!tar.classList.contains("g"))
		return findParentResult(tar.parentElement);
	return tar;
}
