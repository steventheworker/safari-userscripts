...
*/
	...
	function triggerKeyDown(key, modifiers = {}) {
		...
		(tar && tar !== doc.activeElement ? tar : document.body).dispatchEvent(
			ev
		);
	}
	function $isInput(el) {
	...
	const readKeyPath = (obj, keyPath) =>
		objKeyPath.apply(null, [obj, keyPath]);
	...
	const deleteConfig = (keyPath) =>
		deleteKeyPath(Config, keyPath, null, true);
