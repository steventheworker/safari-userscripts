...
const HEADER_RE = /\/\/\s*==UserScript==([\s\S]*?)\/\/\s*==\/UserScript==/m;
...
async function buildOne(dir, isPrivate = false) {
  ...
  const outfile = path.join(OUT, `${name}.user.js`);
  ...
  async function rebuild(label) {
    try {
      ...
      let js = stripHeader(result.outputFiles[0].text);

      // move "use strict" inside the IIFE (() => {...})()
      js = js.replace(
        /^"use strict";\s*\(\(\)\s*=>\s*\{/,
        '(() => {\n\t"use strict";',
      );
